...
// -----------------------------------------------------------------
// Ini jantung dari logika bisnis DANA — sesuai skema:
// 1rb-50rb=2rb, 50rb-100rb=3rb, 100rb-200rb=5rb,
// 200rb-500rb=10rb, 500rb-1jt=20rb
//
...
function hitungAdminDana(nominal) {
  ...
  return 20000; // sisanya: 500rb - 1jt
}
...
danaForm.addEventListener('submit', async (event) => {
  ...
  if (state.metode === 'cod') {
    // Tahap 1: untuk COD, nanti di sini kita akan arahkan ke link
    // WhatsApp (wa.me/...) — akan kita buat di langkah berikutnya
    // setelah form dasarnya jalan dengan baik.
